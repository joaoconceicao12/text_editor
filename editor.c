...
int editorReadKey(){
    ...
    if(c == '\x1b'){
      ...
      if (seq[0] == '[') {
        if(seq[1] >= '0' && seq[1] <= '9'){
          if(read(STDIN_FILENO, &seq[2], 1) != 1) return '\x1b'; //se der erro quer dizer que
          //não ha mais nada para ler do terminal
          if(seq[2] == '~'){
            switch(seq[1]){
...
void editorDelRow(int at){
  if(at < 0 || at >= E.numrows) return; //checks if cursor is on a file line

  editorFreeRow(&E.row[at]);
  memmove(&E.row[at], &E.row[at + 1], sizeof(erow) * E.numrows - at -1); 
  //we overwrite the row we are deleting
  //-1 represents the row we are deleting
  E.numrows--;
  E.dirty++;
}
...
void editorInsertChar(int c){
  ...
  if(E.cy == E.numrows){ //the cursor is on the tilde line after EOF
    editorInsertRow(E.numrows, "", 0); //we append a new line so we can write on it
  }
...
void editorDelChar(){
  ...
  erow *row = &E.row[E.cy];
  if(E.cx > 0){ //In case the cursor is not the start of the line
...
void editorFindCallback(char *query, int key){
  ...
  for(i = 0; i < E.numrows; i++){
    ...
    if (row->render) { // Check if render is not NULL
      ...
      //printf("render is NULL for row %d\n", i); // Debug print
    }
  }
}
